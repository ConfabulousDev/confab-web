...
- **Repo filter fork→root collapsing (CF-491, CF-494)**: `backend/internal/db/repo_filter.go` (`RepoRootExpr` for SELECT projections, `RepoMatchExpr` for WHERE clauses) is the single source of truth for the regex + `COALESCE(session_repos.root_name, extracted)` pattern used by every repo-filter call site (Sessions filter list/match, TILs filter list/match, `api/org_repos.go`, `analytics/org_analytics.go`, `analytics/trends.go`). The fork→root mapping is written by two resolvers in `api/sync.go` (both `handleSyncInit` and `handleSyncChunk`), gated by `validation.ValidateGitInfo` and chained ahead of `db.RecordRepoRoot`:
    - **Primary, definitive (CF-494)**: `db.ResolveForkFromRemotes` in `backend/internal/db/git_remote_resolver.go` consumes CLI-shipped `git_info.remotes` + `tracking_remote` (CF-493). Stamps with `root_source='git_remote'`. Pure function over `db.ParsedGitInfo`; case-fold self-loop check; case-sensitive `tracking_remote` ↔ remote-name match.
    - **Fallback, heuristic (CF-491)**: inline block in `handleSyncChunk` (chunk-only, since pr-link rows live in transcript lines) stamps `root_source='pr_inference'` when a PR link points at a different `owner/repo` than the session's `repo_url`.
    - **Precedence**: both run unconditionally in `handleSyncChunk`; `RecordRepoRoot`'s `WHERE root_name IS NULL` guard means the first to write wins. The git_remote resolver runs first within the same handler, so the stronger signal seals the row. A v1 tradeoff (documented above `RecordRepoRoot`): a prior `pr_inference` row blocks a later `git_remote` write; relax the guard when telemetry shows real divergence.

    When adding a new repo-filter consumer, use the `RepoRootExpr`/`RepoMatchExpr` helpers instead of copy-pasting the regex.
- **Session visibility predicate (CF-495)**: `backend/internal/db/visibility.go::VisibleSessionsCTE(shareAllSessions)` is the single source of truth for "which sessions can this user see." Emits `visible_sessions(id, user_id, owner_email, access_type, shared_by_email)` via UNION ALL (default: owned ∪ private-share ∪ system-share; share-all: every session + the same private-share branch so private_share still wins the priority dedup). Routed through by `analytics.GetTrends` (every aggregation + `aggregateFilterOptions`), `dbsession.buildFilteredSessionsQuery` + `buildSharedWithMeQuery` (pagination dedupes via `dedupedVisibleCTE` — owner > private_share > system_share), `dbsession.queryFilterOptionsScoped`, and `dbtil.queryFilterOptions`. Owner filter applied uniformly via `LOWER(owner_email) = ANY(...)` on the dedup output — no per-CTE owned/shared split. When adding a new visibility-aware reader, route through the helper rather than re-implementing the predicate; org_analytics is the only existing consumer that doesn't yet (admin-gated, different model).
- **Demo identity (CF-483)**: `backend/internal/auth/demo.go` — `BootstrapDemoIdentity`, `AutoImpersonateIfDemo`, `EnforceReadOnly` (chained inside every auth middleware, never mounted standalone), `DemoSessionCookieID` (HMAC-derived shared cookie ID), `RenderDemoBannerScriptTag` (XSS-safe `window.__DEMO_IDENTITY__` injection via `html/template.JSEscapeString`), `IsDemoLoginEmail`, `redirectDemoLoginRejected`, `WithReadOnly` / `ReadOnlyFromContext`. Frontend: `frontend/src/utils/demoIdentity.ts` + `components/DemoBanner.tsx` + `components/ReadOnlyToast.tsx`. **Single env var** `DEMO_IDENTITY_EMAIL` activates the whole feature; when unset every demo-mode predicate short-circuits to today's behavior. See `CONFIGURATION.md` and the security review notes in the `auth/README.md` invariants for B1/B2/D1/D2 fixes. **CF-495 extends the demo-viewer-friendliness to Trends**: `Header.tsx` injects `?owner=<self>` on the Trends link for normal users and skips it for the demo viewer (same pattern as Sessions/TILs from #210), so the demo identity — which owns nothing — lands on a bare `/trends` and gets the aggregate across all visible sessions instead of an empty state.
