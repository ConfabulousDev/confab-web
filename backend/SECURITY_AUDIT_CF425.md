...
|---|----------|----------|---------|
| A1 | High | `internal/auth/oauth.go:1129` | OAuth code-grant lacks PKCE for all three IdPs. MitM with code interception is mitigated only by HTTPS. Self-hosted operators behind a reverse proxy with weak TLS termination are exposed. **→ [CF-426](https://linear.app/confabulous/issue/CF-426)** — ✅ **RESOLVED (r9zn)**: S256 PKCE added to all three IdPs. `code_verifier` (32 random bytes, base64url) is stored in an HttpOnly+Secure `oauth_verifier` cookie and `code_challenge`/`code_challenge_method=S256` are sent on every authorize URL; the verifier rides along in the token exchange. Google and generic-OIDC enforcement is standard. **GitHub is best-effort**: GitHub accepts the PKCE params but its enforcement is not contractually guaranteed — confirm with a one-time live wrong-verifier test against a real GitHub OAuth App before claiming full A1 closure for GitHub (manual, not in CI). |
