...
| `external.go` | External API endpoints (API key auth + dedicated rate limiter): condensed transcript (`GET /sessions/{id}/condensed-transcript`), session file list (`GET /sessions/{id}/files`), session file download (`GET /sessions/{id}/files/download`). Each has a UUID variant and an external_id variant. Shared helpers: `resolveExternalID`, `serveCondensedTranscript`, `serveSessionFiles`, `serveSessionFileDownload` |
