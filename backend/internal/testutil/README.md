...
This runs `go test ... -coverprofile=... -coverpkg=./internal/...` one package
at a time, merges the per-package profiles into `backend/coverage.out`, and
prints a per-package coverage table. Running sequentially
...
- Per-package table for an existing profile, without running tests:
  `COVERAGE_SUMMARY_ONLY=coverage.out ./scripts/coverage.sh`
...
docker load. If it flakes, use `make coverage`. Keep `-coverpkg`: without it
a package is credited only with its own tests, which badly under-reports
packages such as `internal/api` whose HTTP tests live in subpackages.
