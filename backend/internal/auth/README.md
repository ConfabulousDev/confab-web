...
6. **Add tests** following the existing patterns. Keep the provider's endpoint URLs in unexported package-level vars (like `githubTokenURL`/`googleUserinfoURL`) so unit tests can point them at `httptest` fake servers (`oauth_provider_http_test.go`), and add a `Set<Provider>EndpointsForTest` hook to `export_test.go` so the callback integration table in `oauth_callback_integration_test.go` can drive the new provider through every branch (exchange/user-fetch failure, unverified email, demo email, domain, user cap, auto-link, inactive user, happy path, email mismatch).
...
- **Auth rejections are logged as structured WARN lines, never silently.** The session middleware emits one `log.Warn` with a stable `reason` plus request context (`client_ip` via `clientip.FromRequest`, `method`, `path`, and `user_id` where known) at each denial: `TrySessionAuth`'s inactive-user branch (`reason=user_inactive`), `RequireSession`'s final 401 (`reason=no_valid_session`), and its domain 403 (`reason=email_domain_not_permitted`). Ordinary anonymous/expired traffic (no cookie, unresolvable session) stays silent in `TrySessionAuth` to avoid per-request noise — the decisive line is logged once at `RequireSession`. Login-time rejections are logged separately by the callbacks (`OAuth login blocked for inactive user`) and `redirectUserIneligible`. **No session tokens or API keys appear in these logs** (xr71).
...
- **Unit tests** -- `auth_test.go` (API key generation/hashing, context helpers), `oauth_test.go` (OAuth config, CSRF state validation), `oauth_helpers_test.go` (post-login redirect logic, email mismatch), `oauth_helpers_extra_test.go` (`cookieSecure`, `clearCookie`, `handleCLIRedirect` (prefix-only guard), `oauthHTTPClient`, `setOAuthLoginCookies`, `writeDeviceTokenError`), `oauth_callback_test.go` (callback handler patterns), `oauth_extract_test.go` (the extracted `validateOAuthCallback` + `checkUserEligibility` shared helpers — e7py), `password_test.go` (bcrypt, bootstrap admin), `middleware_test.go` (RequireSession, RequireAPIKey, OptionalAuth, RequireSessionOrAPIKey), `localhost_test.go` (localhost URL validation), `oidc_test.go` (OIDC discovery, email_verified parsing), `oidc_http_test.go` (`exchangeOIDCCode` and `getOIDCUser` happy/error paths plus `getOIDCEndpoints` lazy-discovery caching against an `httptest`-backed fake IdP), `oauth_provider_http_test.go` (GitHub/Google token exchange + user fetch against `httptest` fakes, including the primary+verified GitHub email rule; overrides the unexported endpoint vars), `device_html_test.go` (device-page and device-result HTML generators, `GetUserIDContextKey` accessor). `oauth_extract_test.go` also covers `redirectUserIneligible`'s error-code mapping.
- **Integration tests** -- use `testutil.SetupTestEnvironment(t)` and live in package `auth_test` (testutil imports `auth`, so in-package tests cannot use it). `export_test.go` exposes the test-only hooks they need (`SetGitHubEndpointsForTest`, `SetGoogleEndpointsForTest`, `SessionAuthResultForTest`) without adding exported production API. Files: `auth_integration_test.go` (web sessions, API key validation, user cap), `oauth_callback_integration_test.go` (every post-validation branch of the GitHub/Google/OIDC callbacks against fake IdPs), `cli_device_integration_test.go` (`HandleCLIAuthorize`, `HandleDevicePage`, remaining `HandleDeviceToken` branches), `password_demo_integration_test.go` (remaining `HandlePasswordLogin` branches, `AutoImpersonateIfDemo`), `device_verify_integration_test.go`, `demo_integration_test.go`, `auth_logging_integration_test.go`.
- Tests that override the package-level endpoint vars must not call `t.Parallel()`. Create test web sessions with UTC expiries (`time.Now().UTC()`): `web_sessions.expires_at` is a timezone-naive `TIMESTAMP` compared against `NOW()`.
