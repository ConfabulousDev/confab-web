...
| `session.go` | Session listing (`ListUserSessions`, `ListUserSessionsPaginated`), detail retrieval, delete, ownership verification, title/summary updates, ID lookups. Cursor-based pagination, full-text search with `BuildPrefixTsquery`, and filter option materialization. Visibility routes through `db.VisibleSessionsCTE` + `dedupedVisibleCTE` (priority dedup: owner > private_share > system_share) so the predicate is shared with analytics (CF-495). `VerifySessionOwnership` and `GetSessionOwnerExternalIDAndProvider` both return the canonical provider value alongside the external_id so callers can pass it straight into chunk-storage methods. |
