...
| `analyzer_tokens_claude.go` | `TokensAnalyzer` — token counts and estimated cost via `pricing.go` functions. Falls back to `toolUseResult.usage` for agents without files. Also builds the per-model `tokens_v2` tree (7eje): folded into the same group loop, keyed by `getModelFamily()` under the canonical `claude-code` provider, with fast turns under a `"<family> · fast"` key; its `TotalCostUSD` reconciles exactly with the flat `EstimatedCostUSD`. |
