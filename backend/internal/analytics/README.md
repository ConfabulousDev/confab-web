...
| Agents and Skills | `analyzer_agents.go` + `analyzer_skills.go` (two `FileProcessor`s feeding one combined card) | `analyzer_agents_and_skills_codex.go` (no-op stub; Codex has no agent/skill concept) |
...
| `codex_adapter.go` | `ComputeFromCodexRollout` — orchestrator only. Initializes `ComputeResult` and dispatches each `analyzer_*_codex.go` function in order. Per-card mapping decisions for Codex are documented in the orchestrator docstring. |
...
| `analyzer_session_codex.go` | `computeCodexSession` — message counts, breakdown, models used, duration, compactions (all classified as "auto"; Codex doesn't distinguish auto vs manual). |
| `analyzer_tools_codex.go` | `computeCodexTools` — per-tool success/error breakdown. Inline-failed `custom_tool_call` payloads (Status `"failed"`) increment per-tool `Errors` and `ToolErrorCount`. Orphan `<unknown>` synthesized tools are dropped from the per-tool breakdown and excluded from `TotalToolCalls` / `ToolErrorCount`; the anomaly surfaces via `ParsedRollout.ValidationErrors` instead. CF-438. |
| `analyzer_code_activity_codex.go` | `computeCodexCodeActivity` — apply_patch envelope parsing for `FilesModified` / `LinesAdded` / `LinesRemoved` / `LanguageBreakdown`. `FilesRead` stays 0 (Codex has no Read tool). |
...
| `analyzer_agents_and_skills_codex.go` | `computeCodexAgentsAndSkills` — intentional no-op. Documents the empty (agents, codex) and (skills, codex) quadrants. CF-443 will decide whether to skip upserting the empty record entirely. |
...
| `trends.go` | `Store.GetTrends` -- single-user, date-range analytics dashboard. Runs six parallel aggregation queries (overview+activity, tokens, tools, agents+skills, top sessions, providers-present). The tokens path also runs a second `GROUP BY session_type` query via `aggregateTokensPerProvider` for the CF-435 per-provider breakdown; legacy `Claude Code` rows fold into `claude-code` at the Scan site via `models.NormalizeProvider`. `resolveProviderFilter` expands canonical provider values with legacy aliases and defaults to `models.AllowedProviders` so the `session_type = ANY` clause is always present (guards CF-352-style silent omission). |
| `trends_types.go` | Request/response types for the trends API (`TrendsRequest` with `Providers` filter, `TrendsResponse` with top-level `ProvidersPresent`, `TrendsCards`, daily breakdown types, plus `TrendsTokensPerProvider` + `TrendsTokensCard.PerProvider` map for CF-435). |
