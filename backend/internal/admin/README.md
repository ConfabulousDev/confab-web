...
| `card_invalidations.go` | JSON API handlers for date-range card invalidation (`/admin/cards/invalidate`, `/admin/cards/invalidations`, `/admin/cards/types`). `card_types` is validated against `analytics.AllInvalidationTargets` (card tables + `session_title`, nbrd); the optional `providers` must be canonical (`models.CanonicalProviders`) and is alias-expanded before reaching the store. `/admin/cards/types` serves `AllInvalidationTargets`. Execute (`dry_run: false`) requires a `confirm` echo of the affected-session count; the handler re-counts and rejects on mismatch before deleting (kyrr). The audit log details include `providers`. |
| `card_invalidations_test.go` | Integration tests for the card invalidation handlers (incl. `session_title` + provider filter, unknown/legacy provider → 400) |
