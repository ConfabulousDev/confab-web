...
| `created_at` (RFC3339) | same `meta.json` → `createdAtMs` (ms → RFC3339) | Start anchor for estimating session **duration** (`DurationMs` = end − start, where start = `created_at` ?? `first_seen`, end = `last_message_at` ?? `last_sync_at`). Stored by lowering `session.first_seen` when `created_at` is earlier (never raised). Omit when `meta.json` is absent — `first_seen` (set at session init) remains the start anchor. Per-row estimated timestamps for transcript display are interpolated **frontend-side** from these bounds; the stored JSONL is never mutated. |
| `model` | `~/.cursor/.../state.vscdb` → `composerData.modelConfig.modelName`, joined by `composerId == <session-id>` (best-effort; `state.vscdb` is a rotating cache, so the model may be absent for older sessions) | On a transcript chunk a non-empty value is persisted into the `cursor_session_meta` sidecar (PK `session_id`, **first non-empty model wins** — a later chunk never clobbers the first recovered name). The analytics step reads it back so `cards.session.models_used` is `[<model>]`. Absent → no row → `models_used` stays `[]` (never an invented model). Length capped at 255 (over-length → 400). |
| `first_user_message` | first user transcript line (Cursor wraps it in a `<user_query>…</user_query>` envelope) | Unwrapped to the human prompt before validation/storage so the session-list title is clean (the same `<user_query>` extraction the transcript pane, search index, and smart-recap apply). A value with no envelope is stored verbatim; an empty query is dropped. |
...
| `cards.cost_distribution` | object\|null | Log-scale histogram of per-session cost + p50/p90/p99 percentiles over the filtered, visible sessions that carry `tokens_v2` data (y1w5). `null` only on backends predating this card. By default each data point is one session's total cost (`tokens_v2.total_cost_usd`); when `?model=` is active the unit becomes per-(session, selected-model) — each (session, model) pair is one data point and only the selected model's cost counts (the UI flags this with a ⓘ caveat). **Sub-cent (`< $0.01`) data points are excluded entirely** (3tr4) — `$0`/tiny/negative/unpriced values do not appear in any band, the percentiles, or the covered count. Same partial-coverage / timeout-degradation conventions as `cost_by_model`. |
| `cards.cost_distribution.buckets[]` | array | **Dynamic log10 bands**, low→high: the lowest band merges the two sub-`$1` decades into a single `$0.01 – $1` band, and from `$1` up there is one band per power of 10, up to the band containing the most expensive data point — so the top grows with the data (e.g. a `$2M` session yields a `"$1M – $10M"` top band) while empty middle decades are still emitted. There is **no `"< $0.01"` floor band** — sub-cent data points are dropped before bucketing (3tr4); a range with no priced sessions yields `[]`. Each band: `label` (display string, render verbatim — large edges abbreviated e.g. `"$1K – $10K"`), `lo`/`hi` (dollar edges, half-open `[lo, hi)`; `lo` of the first band is `0.01` and its `hi` is `1`, `hi` of the top band is the bounded top decade), `session_count` (data-point count — sessions, or (session,model) pairs under `?model=`), `total_usd` (decimal-string sum across the band). |
| `cards.cost_distribution.stats` | object\|null | `{p50, p90, p99, avg}` of the per-data-point cost values **priced `>= $0.01`** (decimal strings): the percentiles computed with `percentile_cont` (linear-interpolation) semantics, plus the arithmetic mean (`avg`). `null` when there are no priced data points. Biased by the partial `tokens_v2` subset during backfill (the UI caption notes this). |
| `cards.cost_distribution.covered_session_count` | int | Sessions priced `>= $0.01` contributing data points. Sub-cent/`$0`/unpriced sessions are excluded (3tr4), so this is ≤ the count of sessions that merely carry `tokens_v2` data. |
| `cards.cost_distribution.total_session_count` | int | All filtered sessions in range (caption: "Covers N of M sessions priced ≥ $0.01"). |
...
| `filter_options.owners` | string[] | Lowercased owner emails from the caller's **listable** visible sessions, alphabetical. **Static across active filters** — mirrors `SessionFilterOptions` shape on `/api/v1/sessions`. Each option maps to ≥1 listable session (synced lines + a summary/first-user-message), so it never orphans to an empty list (0407). Drives the owner dropdown on TrendsPage without a side-call. CF-495. |
| `filter_options.repos` | string[] | Repo names (`owner/name`) from the caller's **listable** visible sessions, alphabetical. Fork→upstream collapsed via `db.RepoRootExpr`, resolved live per session from its own `git_info` (CF-510). Static across active filters; each option maps to ≥1 listable session (0407). CF-495. |
| `filter_options.models` | string[] | Distinct normalized model-family keys (family + `"· fast"` variants) across the caller's **listable** visible sessions, alphabetical, excluding the empty Unknown key. Static across active filters; sources the model dropdown; each option maps to ≥1 listable session (0407). May be empty/sparse until the `tokens_v2` backfill (`POST /cards/invalidate` + precompute) completes. 2hh1. |
...
| `cards.tokens_v2` | object\|omitted | Hierarchical per-provider/per-model token breakdown. Cached for every session and part of the cache-validity gate, but **included in the response only when it has provider data** — i.e. omitted only for a token-less session. As of 7eje the per-model tree is built for **all** providers (Claude/Codex via the token analyzers, OpenCode via its compute path). Supersedes `cards.tokens` in the UI when present, and is intended to eventually replace it for all providers. |
| `cards.tokens_v2.total_cost_usd` | string | Total estimated cost (decimal as string). For Claude this reconciles exactly with `cards.tokens.estimated_usd` (same per-turn cost incl. fast 6× and server-tool); for Codex it may differ slightly when a session mixes models (v2 prices per-rollout, the flat card prices all at the first model); for OpenCode it is OpenCode's reported per-message cost, falling back to Confab's pricing table for models it reports no cost for. |
...
| `cards.session.models_used` | string[] | Unique model IDs used in the session. Always a JSON array, never null. Cursor has no per-line model, so it emits the single model the CLI sent as `metadata.model` (persisted in the `cursor_session_meta` sidecar), or `[]` when none was sent. |
...
| `pricing` | object | Provider (`claude-code` / `codex`) → model family → rates. Rates are USD per million tokens. `cacheWrite` is the 5-minute cache-creation rate (1.25x input for Claude); `cacheWrite1h` is the 1-hour rate (2x input). When `cacheWrite1h` is absent or `0`, 1-hour cache tokens fall back to the `cacheWrite` rate (so older/remote docs never bill them at $0). OpenAI rows always carry `cacheWrite1h: 0` (no 1-hour tier) and put the cached-input rate in `cacheRead`; their `cacheWrite` is non-zero on the families that publish one (1.25x short-context input on the 5.6 and 6 families) and `0` on those that don't. |
