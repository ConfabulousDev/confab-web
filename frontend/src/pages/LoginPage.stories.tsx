...
function LoginPageLayout({ emailHint, error, showPassword = false, providers = [] }: LoginPageLayoutProps) {
  ...
  function getBtnClass(name: string) {
    switch (name) {
      ...
      <div className={styles.card}>
        ...
        {emailHint ? (
          <p className={styles.subtitle}>
            Sign in with <strong>{emailHint}</strong> to view this shared session
          </p>
        ) : (
          ...
          <div className={styles.oauthButtons}>
            {oauthProviders.map((provider) => (
              <a
                key={provider.name}
                href="#"
                className={getBtnClass(provider.name)}
...
export const AllProviders: Story = {
  args: {
    ...
    providers: [
      ...
      { name: 'github', display_name: 'GitHub' },
      { name: 'google', display_name: 'Google' },
      { name: 'oidc', display_name: 'Okta' },
    ],
  },
};

export const SingleOAuthProvider: Story = {
...
export const WithAccessDeniedError: Story = {
  args: {
    error: {
      type: 'access_denied',
      message: 'This application has reached its user limit.',
    },
    providers: [
      { name: 'github', display_name: 'GitHub' },
    ],
...
export const WithEmailHint: Story = {
  args: {
...
export const PasswordWithEmailHint: Story = {
  args: {
    emailHint: 'alice@example.com',
    showPassword: true,
    providers: [
