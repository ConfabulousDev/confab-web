...
| `cursorTranscriptService.ts` | Cursor transcript: parses the two-shape Cursor JSONL (`{role, message:{content[]}}` / `{type:'turn_ended', …}`), normalizes into `CursorRenderItem[]`, and mirrors the fetch / poll surface. `parseCursorJSONL` keeps malformed lines as `CursorInvalidLine` so line offsets stay aligned for incremental fetch; the normalizer drops `turn_ended` and invalid entries (Cursor MVP has no `unknown` render row), emits one `assistant` item per line plus one `tool` item per `tool_use` block (no output — Cursor records inputs only), and assigns synthetic line-based ids (`${lineIndex}` / `${lineIndex}-${blockIndex}`) since the wire format has no stable id. `extractCursorModel()` is always `undefined` |
