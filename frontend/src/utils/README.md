...
| `externalLinks.ts` | CF-571 — canonical external URLs surfaced from inside the app: `DOCS_URL` (docs site) and `GITHUB_ISSUES_URL` (issue tracker). Point at the hosted project on every deployment, including self-hosted. Used by `Header`, `Footer`, `SessionEmptyState`, `ErrorBoundary`, `LoginPage`. |
