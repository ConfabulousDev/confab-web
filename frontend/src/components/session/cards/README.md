...
| `TokensV2Card.tsx` | Per-model token + cost breakdown from the `tokens_v2` tree (all providers as of 7eje). Top totals (Estimated cost / Input / Output) with the `$0`/unpriced amber warning + green cost styling; then per-model sections (Input / Output / Cache / Reasoning / Cost). Single-provider sessions (Claude/Codex; OpenCode single-vendor) collapse the provider wrapper and render model sections directly; multi-provider keeps `providerLabel()` section headers + a per-provider cost row. Model keys are formatted via the shared `formatModelDisplayName` (mp4e); `"<family> · fast"` keeps its suffix, `""` renders as "Unknown". |
