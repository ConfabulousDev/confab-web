...
| `TokensV2Card.tsx` | Per-model token + cost breakdown from the `tokens_v2` tree (all providers as of 7eje). Top totals (Estimated cost / Input / Output) with the `$0`/unpriced amber warning + green cost styling; then per-model sections. Each model section is a collapsible disclosure (d3rp): the headline is a full-width button (`aria-expanded`/`aria-controls`, keyboard-operable, rotating `ChevronIcon`) showing the formatted model label + its `<CostAmount>`; expanding reveals the Input / Output / Cache read / Cache write / Reasoning detail rows (cache/reasoning gated on `>0`). Cost stays in the always-visible headline. A card with exactly one model section total auto-expands it; with several, all start collapsed (local `useState`, no persistence). Single-provider sessions (Claude/Codex; OpenCode single-vendor) collapse the provider wrapper and render model sections directly; multi-provider keeps `providerLabel()` section headers + a per-provider cost row. Model keys are formatted via the shared `formatModelDisplayName` (mp4e); `"<family> · fast"` keeps its suffix, `""` renders as "Unknown". |
