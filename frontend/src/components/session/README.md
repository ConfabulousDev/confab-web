...
| File | Role |
|------|------|
| `SessionViewer.tsx` | Top-level session viewer with Summary/Transcript tab switching. Provider-agnostic (CF-417): dispatches transcript fetch / filter / rendering through a `ProviderAdapter` resolved via `getAdapter(session.provider)` (see `frontend/src/providers/`). Owns the `isCostMode` toggle and composes the active adapter's `ClaudeFilterDropdown` into `SessionHeader`'s `filterSlot`. The Summary tab is shared (`SessionSummaryPanel`, CF-364). et0r: when the adapter has the `threads` capability, orchestrates subagent subtabs — see "Subagent threads" under Invariants |
| `TranscriptThreadTabs.tsx` | we3k (redesign of et0r's strip): provider-agnostic thread switcher under the Transcript tab. Props `threads`, `activeThreadId`, `sessionId`, `onSelect(threadId, targetId?)`. A `Main` chip is pinned outside a horizontal scroller of subagent chips; both sit in one `role="tablist"` with manual activation (roving `tabIndex`; ArrowLeft/Right/Home/End move focus and scroll it into view; Enter/Space activates). The scroller never shows a native scrollbar (`scrollbar-width: none` + scoped `::-webkit-scrollbar`, overriding `index.css`); hidden chips are signaled by alpha `mask-image` edge fades and ‹ › step buttons on that side only (local `useHorizontalOverflow`: scroll + `ResizeObserver`; buttons are `aria-hidden` mouse affordances, hidden on touch and ≤480px, scrolling 80% of the width), and a vertical wheel scrolls it sideways. Each subagent chip has a `ThreadStatusGlyph`, a ≤40ch label (22ch ≤480px), an indent mark when nested, a status-bearing accessible name ("Label, running"), and a shared `Tooltip` (status, type, model, duration, "Launched by"). The active chip stays in view. Clicking the already-active subagent chip opens its menu: "Go to parent (…)" → `onSelect(parentThreadId, launchTargetId)` (hidden when the parent or launch row is unknown) and "Copy link to this subagent" (`?tab=transcript&agent=<id>`). The menu closes on thread switch, scroll, outside click, and Escape (focus returns to the chip). `AllThreadsDropdown` sits at the right. Smooth scrolling is off under `prefers-reduced-motion` |
| `AllThreadsDropdown.tsx` | we3k: "All subagents (n)" button + listbox (`useDropdown`): Main first, then every thread in strip order (nested indented) with status glyph, full wrapping label, type, "Launched by", model and duration; active row marked. Filter input (label / type / model, case-insensitive, focused on open) when there are more than 8 subagents. ArrowUp/Down highlight (`aria-activedescendant`), Enter/click selects and closes, Escape closes and refocuses the button. ≤480px the button collapses to icon + count |
| `ThreadStatusGlyph.tsx` | we3k: status glyph per `TranscriptThreadStatus` — running ring with a rotating arc (static under reduced motion), check, cross, square, dot; distinct shapes, `data-status`, `aria-hidden` (callers put the word in text) |
| `threadDetails.ts` | we3k: pure helpers for the strip and dropdown — `THREAD_STATUS_LABEL`, `threadStatus`, `isNestedThread`, `parentLabel` ("Main" / parent label / raw id / undefined), `threadMatchesQuery` |
| `ThreadDropdown.module.css` | we3k: shared popover chrome for the active-chip menu and `AllThreadsDropdown` |
| `TranscriptPaneStatus.tsx` | Shared loading / not-synced / error placeholder for the transcript panes. `notSynced` (et0r D7) shows "Subagent transcript not synced yet — retrying" instead of the error while `useTranscriptData` retries a 404'd thread file |
...
| `ClaudeTranscriptPane.tsx` | Transcript tab for Claude Code: thin wrapper around `transcript/claude/ClaudeMessageTimeline` that handles loading / not-synced / error states (parent owns filter + cost state so the header can render chips). Forwards et0r's `activeThreadId` / `onOpenThread` to the timeline |
...
| `OpenCodeTranscriptPane.tsx` | Transcript tab for OpenCode: presentational, virtualized list of user / assistant / tool / unknown render items with the shared `TimelineBar` minimap (ag2x), the green `CostBar` rail in cost mode (hfk7), and Cmd-F in-transcript search (5p9j). Search reuses the shared `useTranscriptSearch` toolkit driven by `extractOpenCodeItemText`. 6h7m: virtual items (real rows + injected day/idle-gap dividers, via `opencodeVirtualItems.ts`) replace the raw `filteredItems` array as the virtualizer's source; a `filteredIndexToVirtualIndex` map (mirroring Claude's `messageIndexToVirtualIndex`) translates deep-link scroll, search-match scroll, and first-visible-index tracking from the filtered-index axis into virtualizer space. The current-match effect scrolls the virtualizer to the match and brings its `<mark>` into view (finds matches in unmounted rows), and force-opens any collapsed reasoning / tool-output `<details>` holding the active match |
| `opencodeVirtualItems.ts` | (6h7m) Pure `buildVirtualItems(items)` — real OpenCode rows + injected dividers via the shared `shouldShowDivider`/`formatDividerLabel` (`timelineUtils.ts`). `timeCreated` is a real, required epoch-ms field, so no divider is ever flagged `estimated` (cf. `cursorVirtualItems.ts`) |
...
interface SessionViewerProps {
  ...
  activeThreadId?: string | null; // et0r: controlled subagent thread (null = Main)
  onThreadChange?: (threadId: string | null, targetId?: string) => void; // presence = controlled threads
  targetId?: string;             // Deep-link target (provider-opaque: Claude UUID, Codex lineId)
  initialMessages?: TranscriptLine[];     // Storybook bypass
  initialThreadMessages?: Record<string, TranscriptLine[]>; // Storybook bypass, per thread id (et0r)
...
- **Subagent threads (et0r)**: only when `adapter.threads` exists. The main transcript is always loaded and polled (the strip and header model/duration derive from it); the strip = `threads.discover(main.items, null)` plus nested threads opened from inside a subagent tab during this visit (we3k: each placed right after its parent in open order, keeping its own label; reset on session change). Nested refs are replaced by `threads.discover(threadItems, parentId)` whenever their parent is the loaded thread (a render-time sync, not an effect), so their status is live while the parent is open and keeps the latest value after switching away. An active thread id with no discovered ref (deep link) still opens, labeled by id via `threads.fileNameFor`, with no Go to parent. A second `useTranscriptData` (`preferCache`) fetches and polls only the active thread; counts, filtered items, `useDeepLinkFilterReset`, and the pane all follow the active thread, while `sessionMeta` stays on Main. Filter state (`?hide=`) is shared across tabs; the pane is keyed per thread so Cmd-F search, scroll, and selection reset on switch. `SessionDetailPage` controls the thread via `?agent=<id>`: `agent` (like `msg`) forces the Transcript tab, leaving Transcript clears both, and a thread change sets/clears `agent` and sets `msg` to the landing row (Go to parent) or clears it. The strip renders only on the Transcript tab with ≥1 thread.
...
- `SessionViewer.test.tsx` -- Summary-tab routing across providers (CF-364); et0r subagent strip (launch order, Summary hides it, controlled/uncontrolled switching with pane items + header counts following the thread, status in chip names, id-labeled deep-link tab without Go to parent, nested chip placed after its parent with its own label, Go to parent from Main-launched and nested threads, nested status refreshed from the parent thread and kept, no strip for Codex)
- `TranscriptThreadTabs.test.tsx` -- we3k: Main outside the scroller, status names + glyphs, no `title`, tooltip details, nested indent, chip menu (open on active chip only, Go to parent, unknown parent, Copy link, Escape focus, closes on switch), roving tabindex + manual activation, scroll-into-view, overflow fades / step buttons / wheel
- `AllThreadsDropdown.test.tsx` -- we3k: count label, rows + details, selection, filter threshold + matching, keyboard, Escape focus
- `TranscriptPaneStatus.test.tsx` -- Loading / not-synced / error / idle
