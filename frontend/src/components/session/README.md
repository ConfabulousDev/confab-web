...
| `cursorCategories.ts` | Cursor render-item types + flat categorization (18n2): `CursorRenderItem` (`user`/`assistant`/`tool`), `CursorFilterState`, `CursorHierarchicalCounts`, `countCursorCategories`, `cursorItemMatchesFilter`, `DEFAULT_CURSOR_FILTER_STATE`. No `unknown` kind — both wire shapes are fully handled |
