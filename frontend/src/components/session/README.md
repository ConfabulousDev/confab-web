...
| `TranscriptThreadTabs.tsx` | et0r: provider-agnostic subtab strip under the Transcript tab (`role="tablist"`): `Main` pinned first, then one tab per `TranscriptThreadRef` in order, horizontally scrollable with truncated labels (full label in `title`); scrolls the active tab into view. Optional `backLink` renders "← Launched from <label>" |
...
- `SessionViewer.test.tsx` -- Summary-tab routing across providers (CF-364); et0r subagent strip (launch order, Summary hides it, controlled/uncontrolled switching with pane items + header counts following the thread, id-labeled deep-link tab, nested open, back link, no strip for Codex)
- `TranscriptThreadTabs.test.tsx` -- Tab roles/order, `aria-selected`, `title`, select callbacks, back link
