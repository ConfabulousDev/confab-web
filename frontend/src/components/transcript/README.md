...
| `claude/claudeAgentIndex.ts` | (Claude, et0r) Pure subagent index over any Claude stream (main or a subagent's file). `buildClaudeAgentIndex(messages)` → `{ agents (by agentId, launch order), byToolUseId }` of `ClaudeAgentInfo` (`agentId`, launching `toolUseId`, `resultMessageUuid`, description (`toolUseResult.description` ?? tool_use input), `subagentType`, model (`resolvedModel` ?? input), status — `running` for `async_launched`, `completed`, `error` for `is_error`, or the latest notification `<status>` — foreground stats, `fileName`, `lastNotificationUuid`). Pairs `Agent`/legacy `Task` tool_use blocks with the user tool_result whose sibling `toolUseResult` (narrow Zod schema) carries `agentId`; results without a paired Agent/Task tool_use are ignored. `parseTaskNotification` / `getTaskNotificationText` (user string content or `queued_command` attachment in `task-notification` mode) / `findNotifiedAgent` (by task id, then tool-use id — background-command notifications match nothing). `agentFileName(id)` is the single frontend owner of the CLI's flat `agent-<id>.jsonl` naming; workflow-run subagents are not indexed |
...
- `claude/claudeAgentIndex.test.ts` -- et0r index: async / sync / error results, legacy `Task`, unpaired or id-less results ignored, launch order, string + queued-command notifications, repeated notifications, unknown notification ids, nested streams; notification parsing.
- `claude/SubagentCard.test.tsx` -- Launch/finished content, stats, title fallback, open action, collapsed raw details + search auto-open.
