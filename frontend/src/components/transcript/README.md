...
| `BashOutput.tsx` | (shared) Terminal-style bash command output with error styling. The output body (`.bashContent`) caps at `maxHeight` (default 400px) and scrolls (`overflow-y: auto`) so long output stays inside the frame and the exit-code / persisted-output footers (rendered as siblings) remain visible. Surfaces Bash `toolUseResult` metadata (CC ≥ 2.1.143) when threaded in: an `interrupted` badge, a persisted-output footer (`Output truncated — N KB persisted to <path>`, path as plain text, no link/fetch), `returnCodeInterpretation` alongside the exit code, a `noOutputExpected` empty-stdout hint, and an `isImage` gate that labels (does not yet decode) image output |
...
| `timelineUtils.ts` | (shared) Provider-neutral helpers shared by all 4 timelines: `shouldShowDivider` (6h7m — pure epoch-ms day-boundary/idle-gap divider decision: `{ show, dayChanged }`, unifying the day-change and pre-existing >5min idle-gap dividers into one divider type) + `formatDividerLabel` (full "Weekday, Month Day" text when `dayChanged`, else the existing today/not-today idle-gap time text), `TIME_GAP_THRESHOLD_MS`, `retryOnAnimationFrame` (virtualizer scroll positioning), `addCmdFListener` (Cmd/Ctrl+F intercept that opens the search bar — CF-359), and `SCROLL_NAV_COST_MODE_RIGHT` (CF-369 — 56px rightOffset both timelines pass to `ScrollNavButtons` in cost mode so the floating buttons clear the CostBar / TimelineBar rail). Kept narrow (decision/format/scroll-retry logic only) — each provider's own `*VirtualItems.ts` builder owns the loop that calls these |
| `TimeSeparator.tsx` | (shared, 6h7m) Presentational divider row (line — label — line), replacing 4 copies of the same markup. Takes `label: string` and an optional `estimated?: boolean` (renders the muted `~` prefix + "estimated" tooltip Cursor uses elsewhere, via the exported `ESTIMATED_TIME_TOOLTIP`). Consumed by all 4 providers' virtualized timelines/panes for both the idle-gap and day-boundary dividers |
| `claude/ClaudeMessageTimeline.tsx` | (Claude) Virtualized timeline (TanStack Virtual) that renders the Claude transcript. Owns selection/scroll state, search, cost bar; virtual-item building (incl. divider injection) is delegated to `claude/claudeVirtualItems.ts`. Mirror of `codex/CodexMessageTimeline`. et0r: builds `buildClaudeAgentIndex(allMessages)` and provides it with `activeThreadId` / `onOpenThread` through `ClaudeThreadContext` so rows and attachment renderers can render subagent cards |
| `claude/claudeAgentIndex.ts` | (Claude, et0r) Pure subagent index over any Claude stream (main or a subagent's file). `buildClaudeAgentIndex(messages)` → `{ agents (by agentId, launch order), byToolUseId }` of `ClaudeAgentInfo` (`agentId`, launching `toolUseId`, `resultMessageUuid`, description (`toolUseResult.description` ?? tool_use input), `subagentType`, model (`resolvedModel` ?? input), status — `running` for `async_launched`, `completed`, `error` for `is_error`, or the latest notification `<status>` — foreground stats, `fileName`, `lastNotificationUuid`). Pairs `Agent`/legacy `Task` tool_use blocks with the user tool_result whose sibling `toolUseResult` (narrow Zod schema) carries `agentId`; results without a paired Agent/Task tool_use are ignored. `parseTaskNotification` / `getTaskNotificationText` (user string content or `queued_command` attachment in `task-notification` mode) / `findNotifiedAgent` (by task id, then tool-use id — background-command notifications match nothing). `agentFileName(id)` is the single frontend owner of the CLI's flat `agent-<id>.jsonl` naming; workflow-run subagents are not indexed. we3k: `normalizeAgentStatus(status)` maps raw statuses to the shared `TranscriptThreadStatus` (`running`, `completed`, `error`/`failed` → `failed`, `killed` → `stopped`, anything else → `unknown`), used by the Claude adapter's thread refs and `SubagentCard` |
| `claude/claudeThreadContext.ts` | (Claude, et0r) `ClaudeThreadContext` (`agentIndex`, optional `onOpenThread`, `activeThreadId`) + `useClaudeThread()`. Default value has an empty index, so rows rendered outside a timeline never show cards |
| `claude/SubagentCard.tsx` | (Claude, et0r) Card for a subagent: `launch` variant (in place of an Agent tool_result) and `finished` variant (task notification, with its own status + summary). Shows title (description → subagent_type → agentId), type chip, model, status pill (we3k: the `normalizeAgentStatus` word — running / completed / failed / stopped / unknown — with info / success / error / neutral styling), foreground stats (duration, tokens, tool uses), an "Open transcript →" button (only when `onOpenThread` is given), and the raw content in a collapsed `<details>` that auto-opens when the row becomes the active search match |
| `claude/claudeVirtualItems.ts` | (Claude, 6h7m) Pure `buildVirtualItems(messages, messageToAllIndex)`, extracted from `ClaudeMessageTimeline.tsx`. Injects a divider (`shouldShowDivider`/`formatDividerLabel`) between messages, comparing each timestamped message against the last message that HAD a timestamp — not strictly the immediately-previous array element — so a timestamp-less `summary` line doesn't hide a real day/gap boundary |
| `claude/ClaudeTimelineMessage.tsx` | (Claude) Single Claude message row — header (role, model, timestamp, cost), content blocks, attachments. Extracts the message's sibling `toolUseResult` as a typed `BashToolResult` via `getBashToolUseResult` (user messages only; skips subagent `agentId` results) and threads it into each tool_result `ContentBlock`. CF-574: on a genuinely-unrecognized message (`isUnknownClaudeMessage` from `claudeCategories` — excludes known-but-unlabeled types like `pr-link`) the otherwise-empty body shows a `ReportUnknownButton` (`surface:'message'`). btxt: that body also renders the local `UnknownRawJson` sub-component — a collapsed-by-default `<details>` wrapping a `CodeBlock` of `stringifyUnknownClaudeMessage(message)`, auto-opened on the rising edge of `isCurrentSearchMatch` (`useOpenOnRisingEdge`). Not the shared `UnknownRawDetails` shell, whose own `<summary>` would duplicate this row's header chrome. et0r (via `useClaudeThread`): a tool_result block whose `tool_use_id` is an indexed Agent/Task launch renders as a `SubagentCard` `launch` (block collapsed under "Raw result"); a user `<task-notification>` for an indexed agent renders a `SubagentCard` `finished` (XML under "Raw notification"); other notifications stay plain text. The `agentId` badge is hidden inside that agent's own thread, and copy-link appends `&agent=<activeThreadId>` in a subagent thread. Filter category, role label, and search text are unchanged |
| `claude/ContentBlock.tsx` | (Claude) Dispatcher that renders content blocks by type (text, thinking, tool_use, tool_result, image, unknown). Uses the shared `renderMarkdownToHtml` helper from `@/utils/markdown`. Accepts an optional `toolUseResult` prop (typed `BashToolResult`) threaded from the message's sibling field; for Bash tool_result blocks it maps the metadata into `BashOutput`. CF-574: the unknown-block fallback pairs its label with a `ReportUnknownButton` (`surface:'content block'`) |
...
| `RowActions.tsx` | (shared, a9gr / CF-360) Provider-agnostic per-row action cluster (sibling of `TimelineBar` / `CostBar`). Renders, into a row's header-right slot, `[prev-skip?] [next-skip?] [copy-text?] [copy-link]`. Copy-link is always rendered; copy-text is hidden when `copyText` is empty/whitespace; skip buttons are hidden when their callbacks are absent. Copy-link URL format: `${origin}/sessions/${sessionId}?tab=transcript&msg=${encodeURIComponent(deepLinkMsg)}`. The deep-link value is a **prop** (`deepLinkMsg`) — whatever the provider's resolver matches: Codex passes the row's ISO 8601 `timestamp` (resolved via `resolveCodexDeepLinkTarget`), Cursor passes the synthetic stable `item.id` (matched directly by `cursorAdapter.useDeepLinkFilterReset`). Consumed by all 8 Codex renderers (`deepLinkMsg={item.timestamp}`) and the Cursor pane (`deepLinkMsg={item.id}`); OpenCode adoption is a follow-up |
| `RowActions.module.css` | (shared, a9gr / CF-360) Button-strip chrome (icon button states, copy-success colour). Moved from `codex/CodexRowActions.module.css` |
| `codex/` | Codex transcript renderers (user, assistant, tool call, turn separator, reasoning placeholder, compaction divider, unknown fallback, virtualized timeline). The turn-based timeline bar is the shared `TimelineBar`; the per-row action cluster is the shared `RowActions`. See "Codex transcript renderers" below |
...
- **tool_result** -- Success/error indicator + content (dispatches to `BashOutput` for Bash tool results, `CodeBlock` for others, or recurses for nested content blocks). For Bash results, the message's sibling `toolUseResult` metadata (interrupted / persisted-output / returnCodeInterpretation / noOutputExpected / isImage) is threaded through to `BashOutput`
...
| `codexVirtualItems.ts` | Pure `buildVirtualItems(items)` that injects a divider (6h7m: day-boundary OR `>5min` idle gap, via the shared `shouldShowDivider`/`formatDividerLabel`) and tags each item with `isNewSpeaker`. The speaker rule is Codex-specific: `tool_call` / `reasoning_hidden` / `turn_separator` / `compacted` / `unknown` items do NOT break user/assistant continuity, so `user → tool_call → user` is the same speaker. Also exports `skipNavKey` / `skipNavLabel` (CF-360) — the chain-key + aria-label mapping used for skip-to-next-of-same-kind navigation |
...
| `codexToolCallHelpers.ts` | Pure helpers extracted from `CodexToolCallBlock.tsx` for testability: `buildToolCallCopyText` (per-tool copy-text composition for the shared `RowActions`; CF-368 routes `update_plan` through the same summary text the body renders), shape readers `readStringField`, `readPatchChanges`, `readWebSearchQueries`, `stringifyGenericInput`, and CF-368's `readPlanSummary` + `buildPlanSummaryText` (classify an `update_plan` payload into the `empty` / `complete` / `in_progress` / `paused` / `pending` bucket and render the one-line summary; shared with the search-index projection so renderer and Cmd-F stay in sync) |
...
| `CodexUnknownItem.tsx` | Forward-compat fallback. Thin wrapper over the shared `UnknownRawDetails` shell showing raw JSON for any line whose top-level `type` or nested `payload.type` doesn't match a known schema. Accepts `isSelected` + `isDeepLinkTarget` + `sessionId`; copy-text uses `stringifyForDisplay(rawLine)`. CF-574: renders a `ReportUnknownButton` fed `{provider:'codex', type: item.unrecognizedType, reason}` — `unrecognizedType` + `reason` (`CodexUnknownReason`) are set at each classification site in `codexTranscriptService` (see `CODEX_UNKNOWN_REASON_LABELS` in `types/codexRenderItem`) |
...
| `CodexMessageTimeline.module.css` | Container (flex with bar on the right), virtualizer slot positioning, scroll chrome, and CF-362 `.costBarWrapper` / `.costBarWrapperVisible` slide-in for the cost bar. The divider row's own styling now lives in the shared `TimeSeparator.module.css` (6h7m) |
...
forward-compat. a9gr promoted `CodexRowActions` to a provider-agnostic
`transcript/RowActions` (the hardcoded `msg=<timestamp>` became a
`deepLinkMsg` prop): all 8 Codex renderers now pass `deepLinkMsg={item.timestamp}`,
and the Cursor pane reuses the same component with `deepLinkMsg={item.id}`.
6h7m unified the previously Claude/Codex-only `>5min` idle-gap divider with a
new day-boundary divider (full "Weekday, Month Day" label, shown even for a
sub-5min gap that crosses midnight) into one divider type, driven by the
shared `shouldShowDivider`/`formatDividerLabel` (`timelineUtils.ts`) and
rendered by the shared `TimeSeparator` component, across all 4 providers.
Claude's inline virtual-item builder was extracted to
`claude/claudeVirtualItems.ts` to match Codex's pattern. Cursor and OpenCode
previously had no separator/divider rows at all — both panes gained a
`filteredIndexToVirtualIndex` map (mirroring Claude's pre-existing
`messageIndexToVirtualIndex`) so deep-link scroll, search-match scroll, and
first-visible-index tracking still land correctly once a divider can precede
any row. Cursor's dividers are always `estimated` (its timestamps are
interpolated, per ce79); OpenCode's and Codex's never are.
...
- **`transcript/TimelineBar`** (shared, Codex + OpenCode + Cursor) is the generic `TimelineBar<S extends SpeakerSegment>`. Same user (blue) + assistant (warm orange) palette per turn; tools fold into the assistant stripe (2-color). Tooltip wording is `User: <dur>, 1 item` / `<assistantLabel>: <dur>, N items` (`assistantLabel` is a required prop — `"Codex"` / `"Assistant"`) — no TTFT, no turn index; an optional `tooltipNote` adds a muted disclaimer line (Cursor: `"Estimated times"`). Each consumer supplies its own segment derivation: Codex's user segment is sized by the wall-clock gap from the prior separator (synthetic 1s for turn 1 / zero-gap turns), assistant by the separator's `durationMs`; OpenCode synthesizes turns from user-item transitions (no separator) and sizes both stripes from epoch-ms `timeCreated` deltas; Cursor mirrors OpenCode but parses ISO-string `timestamp`s (estimated, ce79) and yields zero segments — hiding the bar — when any row's time is unknown. Same sizing math and position indicator as Claude.
...
- `ContentBlock.test.tsx` -- Block-type dispatch (text/JSON/thinking/tool_use/tool_result/image/tool_reference/unknown), plus `toolUseResult` threading into `BashOutput` for Bash results (and its no-op for non-Bash). `CodeBlock` and `BashOutput` are stubbed via `vi.mock` so the dispatch logic is asserted without paying the Prism cost.
- `BashOutput.test.tsx` -- Bash metadata rendering per state: interrupted badge, persisted-output footer (humanized size + plain-text path), `returnCodeInterpretation` near the exit code, `noOutputExpected` hint, `isImage` gate/label, and exit-code footer. Stories in `BashOutput.stories.tsx` (one per state).
...
- `claude/claudeAgentIndex.test.ts` -- et0r index: async / sync / error results, legacy `Task`, unpaired or id-less results ignored, launch order, string + queued-command notifications, repeated notifications, unknown notification ids, nested streams; notification parsing; `normalizeAgentStatus` mapping (we3k).
- `claude/SubagentCard.test.tsx` -- Launch/finished content, stats, title fallback, open action, collapsed raw details + search auto-open, normalized status wording + styling (failed, stopped, running, completed).
- Additional integration via `claude/ClaudeTimelineMessage.test.tsx` (incl. et0r subagent cards, badge hiding, `agent` copy-link) and Storybook stories (`claude/SubagentCard.stories.tsx`, (`claude/ContentBlock.stories.tsx`, `CostBar.stories.tsx`, `claude/TimelineBar.stories.tsx`).
