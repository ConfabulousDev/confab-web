...
| `Tooltip.tsx` | we3k shared themed tooltip: `<Tooltip content={…} disabled?>{focusableTrigger}</Tooltip>`. Shows after a ~300ms hover or immediately on keyboard focus (not on a click's focus); hides on pointer leave/press, blur, Escape and any scroll. Sets `aria-describedby` on the trigger while shown (`role="tooltip"`). Rendered in a fixed-position portal on `document.body`, positioned below the trigger, clamped to the viewport and flipped above when needed, so scroll containers and masks never clip it. Events are handled on a `display: contents` wrapper, so the trigger's own props are untouched. Fade-in only without reduced motion. First consumer: the transcript thread strip chips; existing native `title` tooltips have not migrated |
